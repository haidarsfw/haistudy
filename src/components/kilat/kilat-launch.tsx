  ...
  return (
    <div className="mx-auto max-w-3xl py-6">
      ...
      <motion.div variants={staggerContainer(0.05)} initial="hidden" animate="visible" className="mt-4 grid gap-2 sm:grid-cols-2">
        {perChapter.map((ch, i) => {
          // A lone last chapter (odd count) spans the 2-col row but stays at a
          // single card's width, centered — so it isn't a stretched, empty-looking
          // full-width card. Content stays left-aligned like the others.
          const lastOdd = perChapter.length % 2 === 1 && i === perChapter.length - 1;
          return (
            <motion.div
              key={ch.n}
              variants={staggerItem}
              className={`flex items-center gap-3 rounded-xl border border-border bg-card px-4 py-3 ${lastOdd ? "sm:col-span-2 sm:mx-auto sm:w-[calc(50%-4px)]" : ""}`}
            >

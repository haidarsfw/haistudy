...
export function VoicePanel({
  ...
  // Panel should be rendered (in DOM) when:
  // 1. isOpen (user wants to see it) OR
  // 2. activeRoom exists (user is connected, keep VoiceRoom alive)
  const shouldRender = isOpen || !!activeRoom;
    ...
    <>
      ...
      */}
      <motion.div
        ...
        className="fixed right-0 bottom-0 z-50 flex w-full flex-col border-t border-border bg-background shadow-xl max-h-[calc(100dvh-3.5rem)] rounded-t-2xl sm:top-14 sm:bottom-0 sm:right-0 sm:h-auto sm:w-[336px] lg:w-[400px] sm:max-h-none sm:rounded-none sm:border-l sm:border-t-0"
        style={{ 
          // When not open, make it non-interactive but keep it in DOM
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {showCreate ? (
            ...
            <Button
              ...
              onClick={() => {
                if (!VOICE_ENABLED) {
                  toast.info(VOICE_DISABLED_MESSAGE);
                  return;
                }
                if (!guard("preview.voice_blocked")) return;
                sounds.toggle();
                setShowCreate(true);
              }}
            >
